...
class GioHang_DAO implements DAOInterface
{
    ...
    private function createGioHangModel(array $rs): GioHang
    {
        ...
        return new GioHang($id, $email, $createdAt, $trangThaiHD);
    }
    ...
    {
        $list = [];
        $query = "SELECT * FROM giohang WHERE EMAIL = ? AND TRANGTHAIHD = 1";
    ...
    {
        $query = "UPDATE giohang SET EMAIL = ?, CREATEDAT = ?, TRANGTHAIHD = ? WHERE ID = ?";
        $args = [$e->getEmail(), $e->getCreatedAt(), $e->getTrangThaiHD(), $e->getId()];
    ...
    public function controlDeleteModel(int $id,int $active): int
    {
        $query = "UPDATE giohang SET TRANGTHAIHD = ? WHERE ID = ?";
        $rs = database_connection::executeUpdate($query, $active,$id);
        return is_int($rs) ? $rs : 0;
    }
    /**
